...
% newRelease
%   Prepares a new release of the Papla model, for direct submission to
...
%Bump version number:
oldModel   = load('../ModelFiles/mat/papla.mat');
...
fclose(fid);
if ~contains(history,['paplav' newVersion ':'])
...
model.description = ['Papiliotrema laurentii-GEM_v' newVersion];

%Save model
exportForGit(model,'papla','..',{'mat', 'txt', 'xlsx', 'xml', 'yml'});
